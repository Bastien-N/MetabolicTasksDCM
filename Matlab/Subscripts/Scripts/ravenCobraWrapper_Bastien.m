function newModel = ravenCobraWrapper_Bastien(model,RavenPath)

% ravenCobraWrapper_Bastien
%   Converts between RAVEN and COBRA structures, modified to fix a
%   metabolite naming bug by Bastien Nihant, only fixed to work in the
%   specific case of Human1
%
%   Input: model          a RAVEN/COBRA-compatible model structure
%             RavenPath    The absolute path to the RavenToolbox
%             installation, required to be able to keep this script out of
%             Raven directories0
%
...
warning('Please advise: this script was not fixed to work with models other than Human1v 1.12 & 1.13')
...
if nargin < 2
    error('You HAVE to provide your RavenToolbox installation path');
end
%RavenPath='C:\Users\asus\Documents\RAVEN';%findRAVENroot();
...
if isRaven
    ...
    if isfield(model,'metNames')
        newModel.metNames=model.metNames;%strcat(model.metNames,' [',model.compNames(model.metComps),']');
    end
    ...
    %NOT NEEDED FOR HUMAN1
%     for i=1:numel(model.comps)
%         newModel.mets=regexprep(newModel.mets,['\[', model.comps{i}, '\]$'],'');
%         newModel.mets=regexprep(newModel.mets,['\[', model.compNames{i}, '\]$'],'');
%     end
    for i = 1:length(newModel.mets)
        newModel.mets{i} = newModel.mets{i}([1:8 10]);
    end
    %BELLOW WOULD NOT REPLICATE THE BASE HUMAN1 FORMULATION
...
%     end
    ...
    if ~isfield(model,'rev')
        newModel.rev = zeros(size(model.rxns));
    ...
    %WE WANT TO KEEP ANY MODIFICATION TO b
    newModel.b= model.b;%zeros(numel(model.mets),1);
    %COMPS SHOULD JUST BE THE SAME AS IN MODEL
...
%     end
    ...
    %SIMPLIFIED SINCE WE SHOW HUMAN1
    newModel.metComps= newModel.mets;%regexprep(model.mets,'^.+\[','');
    for i = 1:length(newModel.metComps)
        newModel.metComps{i} = newModel.metComps{i}(end);
    end
